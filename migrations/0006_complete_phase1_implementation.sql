-- Migration: Complete Phase 1 Task Management Implementation
-- Version: 0006
-- Description: Adds all missing columns and tables for Phase 1 completion

-- ================================================
-- STEP 1: ALTER EXISTING TASKS TABLE
-- ================================================

-- Add missing columns to tasks table (ignore errors if columns already exist)
ALTER TABLE tasks ADD COLUMN created_by VARCHAR(255);
ALTER TABLE tasks ADD COLUMN owner_id VARCHAR(255);
ALTER TABLE tasks ADD COLUMN last_updated_by VARCHAR(255);
ALTER TABLE tasks ADD COLUMN estimated_hours DECIMAL(6,2);
ALTER TABLE tasks ADD COLUMN actual_hours DECIMAL(6,2);
...
ALTER TABLE tasks ADD CONSTRAINT fk_tasks_owner_id FOREIGN KEY (owner_id) REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE tasks ADD CONSTRAINT fk_tasks_last_updated_by FOREIGN KEY (last_updated_by) REFERENCES users(id) ON DELETE SET NULL;

-- ================================================
-- STEP 2: CREATE NEW TASK MANAGEMENT TABLES
-- ================================================
...
-- Auto-follow tasks for existing assignees and creators
